...
from experiments.system_owned_provenance_recommendation_action_value.judge import JudgeOutcome
from experiments.system_owned_provenance_recommendation_action_value.runner import (
...
def _perfect_results_by_run_id():
    ...
    for entry in build_reasoning_plan(benchmark):
        ...
        results[entry.output_id] = SystemProvenanceRecommendationActionResult(
...
def test_complete_fake_vertical_slice_preserves_system_provenance_and_state(tmp_path) -> None:
    runtime = PerfectRuntime(_perfect_results_by_run_id())
    judge = PerfectJudge()
    output_dir = tmp_path / "spec019"
    ...
    assert result["specification"] == "019-v0.1"
    ...
    assert result["system_provenance_plan_sha256"]

    invariants = result["technical_invariants"]
    for key, value in invariants.items():
        if key in {
            "SPRA-INV-16_complete_fake_design",
            "SPRA-INV-17_ci_no_live_credential",
            "SPRA-INV-20_live_launch_authorization",
        }:
            continue
        assert value is True, key

    provenance_plan = json.loads(
        (output_dir / "system_provenance_plan.json").read_text(encoding="utf-8")
    )
    assert len(provenance_plan) == 36
    generic = [
        item for item in provenance_plan if item["provenance"]["condition"] == "GENERIC"
    ]
    selective = [
        item for item in provenance_plan if item["provenance"]["condition"] == "SELECTIVE"
    ]
    full = [
        item for item in provenance_plan if item["provenance"]["condition"] == "FULL_HORIZON"
    ]
    assert len(generic) == len(selective) == len(full) == 12
    assert all(item["provenance"]["supplied_revisions"] == [] for item in generic)
    assert all(2 <= len(item["provenance"]["supplied_revisions"]) <= 3 for item in selective)
    assert all(len(item["provenance"]["supplied_revisions"]) == 10 for item in full)

    reasoner_records = [
        json.loads(line)
        for line in (output_dir / "reasoner_attempts.jsonl")
        .read_text(encoding="utf-8")
        .splitlines()
        if line.strip()
    ]
    successful = [item for item in reasoner_records if item["status"] == "SUCCESS"]
    assert len(successful) == 36
    for record in successful:
        assert "system_context_provenance" in record
        assert "methodological_basis" not in record["structured_result"]
        provenance = record["system_context_provenance"]
        assert provenance["methodology_payload_sha256"] == record["context_sha256"]
        assert provenance["methodology_payload_bytes"] == record["context_utf8_bytes"]
