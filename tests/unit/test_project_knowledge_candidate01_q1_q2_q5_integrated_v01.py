...
def test_post_promotion_and_result_publication_phase_changes_do_not_rewrite_preserved_evidence():
    ...
    assert recomputed_eval["checks"]["current_authority_unchanged"] is False
    assert set(recomputed_eval["failed_checks"]) == phase_dependent
    final = load(FINAL)
    assert final["integrated_q1_q2_q5_shadow_support"] is True
