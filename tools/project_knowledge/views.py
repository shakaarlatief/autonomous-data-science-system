...
from .model import (
    ...
    RebuildabilityClass, SubstrateError, ViewBuildResult, ViewFreshnessResult, ViewFreshnessStatus,
    ViewGenerator, ViewInput, ViewInputSelector, ViewSpecification, thaw_json, validate_source_path,
)
...
# Data-only executable qualification: identity, source path, function name,
# (local alias, helper identity) dependencies, capability names. No live object
# can register itself. This literal is checked and imported only inside the TCB.
PURE_UNIT_REGISTRY = (
...
def source_inventory_specification() -> ViewSpecification:
    """Small G009 structural view with an explicit repository implementation closure.

    This does not implement the G010 current-state core or publish any output.
    The list includes discovery/parsing/schema dependencies as well as the pure
    builder. It excludes every generated output and manifest.
    """
    files = (
        ...
        "tools/project_knowledge/adapters/pure.py", "tools/project_knowledge/pure_units.py",
    )
