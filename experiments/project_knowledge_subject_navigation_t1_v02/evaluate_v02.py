...
CATALOG = HERE / "subject_catalog_candidate_v02.json"
ANNOTATIONS = HERE / "corpus_annotations_v02.json"
SCENARIOS = HERE / "navigation_scenarios_v02.json"
RESULT_JSON = HERE / "result_v02.json"
PROJECTION_JSON = HERE / "candidate_projection_v02.json"
RESULT_MD = HERE / "RESULT_V02.md"
...
def main():
    ...
    required_policy = {
        ...
        "preferred_subject_rule",
        "null_preferred_rule",
        "preferred_parent_semantics",
        "broader_semantics",
        "parent_assignment",
        "legacy_topics_semantics",
        "membership_ownership",
    }
    assert required_policy <= set(catalog["membership_policy"]), catalog["membership_policy"]
    for subject in subjects:
        assert subject.get("description"), subject["id"]
        if subject["assignable"]:
            assert subject.get("include_when"), subject["id"]
            assert subject.get("exclude_when"), subject["id"]
    ...
    tick = chr(96)
    lines = [
        "# W5 T1 V0.2 Controlled Subject Vocabulary Result",
        "",
