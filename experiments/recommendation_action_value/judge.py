"""Condition-blinded semantic judge for frozen Specification 015.

Exact action dispositions and blocking-scope metrics are evaluated
deterministically by the harness. This judge evaluates only the preregistered
semantic obligations around rationale, dependencies, unresolved context, and
consistency with the supplied project evidence.
"""
...
from experiments.recommendation_action_value.harness import JudgeResult
...
class OpenAIAgentsRecommendationJudge:
    """No-tool semantic judge over a condition-blinded recommendation payload."""
    ...
    def runtime_version(self) -> str:
        try:
            ...
            raise RuntimeError(
                "openai-agents is required for the live recommendation judge; "
    ...
    async def judge(self, *, judge_id: str, payload: Mapping[str, object]) -> JudgeOutcome:
        ...
        agent = agents.Agent(
            name="ADS blinded recommendation rationale judge",
            instructions=(
                ...
                "item marked critical receives score 0. Judge the candidate recommendation "
                "only against the user task, project evidence, visible action/scope/" 
                "clarification menus, and frozen semantic rubric. Do not infer or speculate "
                "about which experimental condition produced the candidate. Exact expected "
                "disposition labels are intentionally not supplied to you and are evaluated "
                "separately by deterministic software."
            ),
        ...
        if not isinstance(final_output, JudgeResult):
            raise ValueError(
                "recommendation judge did not return JudgeResult; "
    ...
    def _raw_usage_entries(result: Any) -> list[dict[str, Any] | None]:
        ...
        for response in result.raw_responses:
            ...
            entries.append(
                None if raw is None else json.loads(json.dumps(raw, sort_keys=True))
            )
        return entries
    ...
    def _normalize_usage(
        ...
        return ReasoningUsage(
            ...
            service_tier=OpenAIAgentsRecommendationJudge._service_tier(raw_entries),
