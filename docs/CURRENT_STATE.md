...
Checkpoint 320 is the current boundary. Research 119 remains the governing scope correction, and Research 120 governs the automatic hybrid PDF direct-source architecture. The live Codexless runtime is now `0.1.1-preview.16-hybrid-pdf-access` / 60 tools with tunnel health/readiness HTTP 200. After refreshing the developer MCP app, a fresh disposable ChatGPT conversation discovered and executed `codex.pdf_access` successfully on `32.LinearModels2.annotated.pdf` with `intent=native`. The facade automatically selected `native-parts`, generated the previously qualified deterministic parts for source pages 1-34 and 35-38, returned two MCP PDF `resource_link` items with no inline PDF bytes/base64, and the ChatGPT host materialized both as actual conversation PDF files. Ordinary ChatGPT-side PDF inspection of the first materialized part then succeeded and confirmed its 34-page structure plus concrete first-page visual/content features. This closes the fresh-host native split route end to end. The next bounded Research 120 qualification is the mixed text/vision behavior on `51.Deep Learning2.annotated.pdf`, especially individually oversized native pages 16 and 49.
