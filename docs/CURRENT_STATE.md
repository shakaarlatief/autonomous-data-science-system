...
Checkpoint 338 is the current boundary. Research 120 remains qualified and closed for the accepted PDF direct-source routing scope, while Research 121 governs the Office whole-file handoff work. AB-020 remains live-qualified and closed. The controlled restart is complete and independently verified: Codexless now reports `0.1.1-preview.17-office-file-link`, 61 tools and `codexless-public-preview-v2`; tunnel `/healthz` and `/readyz` are both HTTP 200. Independent source re-reads confirm the new Office reader/store hashes and the AB-020 semantic-Git hash exactly match the qualified publication. A dynamic `file_link` discovery query in this persistent conversation still does not expose the newly added `codex.file_link`, so the local runtime is classified LIVE while this conversation retains the already-known stale callable projection. The exact next action is now the repository-governed ChatGPT app refresh, followed by a fresh disposable conversation for read-only `codex.file_link` discovery. If the new action is exposed there, run the three-format deterministic DOCX/PPTX/XLSX host-materialization/fidelity matrix; do not substitute an invocation in this stale persistent conversation.
