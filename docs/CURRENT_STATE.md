...
Checkpoint 334 is the current boundary. Research 120 remains qualified and closed for the accepted PDF direct-source routing scope, while Research 121 governs the Office whole-file handoff work. AB-020 is now live-qualified and closed for its concrete tracked-path enumeration defect: after restart, a real private semantic push succeeded at 32,813 tracked-path bytes, above the old 32 KiB outer response ceiling, with `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS`; the complete Office candidate was then privately preserved and pushed at `fa5cc2a6c3e6d47f45961ab475c2ac66c24aff0b`, where the repository now has 503 tracked files / 40,013 tracked-path bytes and still passes the integrity gate. The durable `codex.file_link` candidate is now guarded-publication-preflight-qualified for `0.1.1-preview.17-office-file-link` / 61 tools. Its 10/10 focused suite, seven staged public regressions and 8/8 AB-020 suite all pass. The publication helper is prepared with exact old/new hashes, timestamped backups, rollback, and explicit preservation of the AB-020-qualified semantic-Git source. No preview.17 live file has yet been modified. The exact next action is ordinary-host publication, then independent installed-hash verification, the controlled restart, and fresh ChatGPT discovery before the DOCX/PPTX/XLSX host fidelity matrix.
