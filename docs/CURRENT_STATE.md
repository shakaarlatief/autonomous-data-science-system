...
Checkpoint 314 is the current boundary. Research 119 remains the governing scope correction: ordinary ChatGPT should gain direct bounded access to authorized local-machine files through Codexless without requiring manual upload or a reasoning-model intermediary. The Checkpoint 313 renderer-only candidate is now end-to-end qualified after the controlled restart. The existing `codex.document_render` tool successfully rendered page 16 directly from the 78,874,939-byte read-only `51.Deep Learning2.annotated.pdf` source and delivered a 583,130-byte 1240x1755 standard MCP PNG to ChatGPT, which directly inspected concrete StyleGAN visual content. No Plugin refresh, Browser, OCR, source-workspace write, manual upload, or reasoning-model intermediary was used.
...
The temporary exact-root `codexless-live` ordinary workspace admission used for Checkpoint 313 publication was removed after verification. The durable registry is now revision 9 / content hash `12a82d711f0a4ec71eebc67acf5ed46ed2c0530481a88bb6f93fa51c55266dd0` with only `ads-public`, `ads-local-runtime`, `big-data-statistics`, and `machine-learning`.

The private local-runtime repository is synchronized exactly to `origin/main` at `889148f7263bb4ce9baddf52ce079a3bc2c943db` with `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS` and `postflightOk=true`. It now preserves the Checkpoint 314 end-to-end large-PDF page-render qualification in addition to the Checkpoint 313 candidate/publication evidence and earlier host-capability, Astra and Browser history.

The deterministic multi-native-PDF `document_file_link` qualification remains a real fallback, but Validation 070 prevents treating it as universal. The first oversized-single-page visual route is now fully qualified. On the real 78,874,939-byte `51.Deep Learning2.annotated.pdf`, page 16's 15,944,609-byte native one-page PDF was bypassed by direct model-free rendering to a 583,130-byte 1240x1755 PNG. After the controlled restart, that exact PNG reached ChatGPT through `codex.document_render`; its SHA-256 matched the pre-restart feasibility probe exactly, and ChatGPT directly inspected the StyleGAN architecture, latent-vector/per-layer-noise labels, generated-face grid and latent-vector manipulation section. The next direct-access problem is generalizing the hybrid policy, especially a first-class large-source text path and automatic dispatch among whole-file, native-split and page-render fallbacks.
...
The workspace registry is now revision `9`, content hash `12a82d711f0a4ec71eebc67acf5ed46ed2c0530481a88bb6f93fa51c55266dd0`, with `ads-public`, `ads-local-runtime`, `big-data-statistics`, and `machine-learning`. Temporary exact-root `codexless-live` read/write admissions have been used only for bounded one-time live publication work, most recently Checkpoint 313, and removed immediately after publication/test verification. `big-data-statistics` has only `read` and `agent`; `machine-learning` remains strictly `read` only. Neither personal workspace has write, browser, or Git capability. Exact personal roots remain machine-local rather than public project authority.
...
The stable architecture now supports explicit register/update/remove of ordinary filesystem/project roots without another MCP schema publication. Per-workspace capability checks remain server-owned; semantic Git selects only `workspaceId`, derives branch/upstream dynamically, preserves the registered remote and integrity policy, and exposes no caller-selected cwd/URL/refspec/credentials/config/profile/sandbox/force inputs. Authenticated private Git is proven through the bounded host-network substrate. The latest preserved local-runtime boundary is `889148f7263bb4ce9baddf52ce079a3bc2c943db`. It preserves the completed large-PDF page-render qualification, including the real Machine Learning page-16 feasibility evidence, guarded installed-source publication, rollback metadata and same-chat end-to-end PASS, while retaining the earlier host-capability, Astra Phase 2 and GPT-5.6 Sol Browser evidence in private history. The latest private push passed `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS`, exact local/remote equality, clean tracked postflight, and `postflightOk=true`.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `large-pdf-page-direct-render-qualified` boundary unless the repository has advanced further. The oversized-single-page visual route is now proven end to end on the real 78,874,939-byte Machine Learning source. Do not repeat that qualification. The next bounded research step is to generalize the hybrid direct-source policy, with particular attention to a first-class large-source text path and automatic selection among unchanged whole-file handoff, native PDF parts and rendered-page fallback. For operational continuation steps, the collaborator must also resolve and read the governing procedure referenced by the active boundary before giving exact execution instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
