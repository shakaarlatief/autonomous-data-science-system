...
Checkpoint 337 is the current boundary. Research 120 remains qualified and closed for the accepted PDF direct-source routing scope, while Research 121 governs the Office whole-file handoff work. AB-020 remains live-qualified and closed. The corrected guarded preview.17 `-Publish` attempt has now succeeded. The helper reported `OFFICE_FILE_LINK_PUBLICATION_RESULT=PASS`, all eight live-disk regression scripts passed, and `AB020_SEMANTIC_GIT_PRESERVED=true`. A separate read-only probe independently re-read all nine Office publication targets and the AB-020 semantic-Git source; every SHA-256 matches the exact qualified candidate and `AllHashesMatch=true`. The new `file-link-reader.mjs`, `file-resource-store.mjs`, and `file-link-regression.mjs` are present on disk, while the six replaced source/test files match their preview.17 hashes. The currently running process and tunnel are intentionally still preview.16 / 60 tools with HTTP 200/200 because `RESTART_PERFORMED=false`; therefore the new tool surface is source-published but not process-live. The exact next action is the full controlled restart under `docs/local_execution/OPERATIONS.md`, requiring local health to report `0.1.1-preview.17-office-file-link` / 61 tools before restarting the tunnel. Because the tool surface changed, the existing ChatGPT developer MCP app must then be refreshed and a fresh disposable chat used for read-only `codex.file_link` discovery before the DOCX/PPTX/XLSX host fidelity matrix.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `office-source-published-restart-next` boundary unless the repository has advanced further. Research 120 is complete for its accepted automatic PDF architecture scope; Research 121 governs the current Office whole-file handoff work. The exact next action is ordinary-host publication of the already-qualified preview.17 helper. After publication, independently verify the nine installed file hashes, then follow the full controlled restart sequence in `docs/local_execution/OPERATIONS.md`. Because the public surface changes to 61 tools, fresh ChatGPT discovery is required before the DOCX/PPTX/XLSX materialization/fidelity matrix. AB-020 no longer blocks new tracked candidate paths. Do not widen the generic `command/exec` output cap or weaken private secret/binary/content gates. Preserve the `--allow-addons` warning observed during cached PDF rendering as a hardening caveat. AB-008 remains relevant to fresh-chat schema acquisition. For operational continuation steps, the collaborator must resolve and read the governing procedure referenced by the active boundary before giving exact restart/publication instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
