...
Checkpoint 313 is the current boundary. Research 119 remains the governing scope correction: ordinary ChatGPT should gain direct bounded access to authorized local-machine files through Codexless without requiring manual upload or a reasoning-model intermediary. Checkpoint 312 established that native page-range splitting is not universal for the larger read-only Machine Learning PDFs. A narrow renderer-only response is now statically qualified and source-published: `codex.document_render` retains its existing schema but may admit authorized PDF sources up to 96 MiB while the parent process validates identity with bounded streaming hashes instead of retaining two whole-file snapshots. The live process restart and end-to-end page-16 ChatGPT image qualification are the only remaining steps for this candidate.
...
The private local-runtime repository is synchronized exactly to `origin/main` at `e96d53248de6718d14072dd857325a15587be5dc` with `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS` and `postflightOk=true`. It now preserves the Checkpoint 313 large-PDF page-render candidate, static qualification, guarded live-source publication evidence and rollback identities in addition to the earlier host-capability, Astra and Browser history.

The deterministic multi-native-PDF `document_file_link` qualification remains a real fallback, but Validation 070 prevents treating it as universal. The first oversized-single-page sub-route is now ready for live qualification. On the real 78,874,939-byte `51.Deep Learning2.annotated.pdf`, model-free PDF.js + canvas already rendered page 16 to a 583,130-byte 1240x1755 PNG and extracted 17,999 embedded-text characters under the same 256 MiB Node heap. The installed renderer source has therefore been changed narrowly to use a 96 MiB render-source ceiling with bounded streaming identity hashes while preserving the existing 4 MiB per-page and 8 MiB aggregate image ceilings. Installed renderer regression is 10/10 PASS. The currently running process still needs the controlled restart before this becomes live end-to-end evidence.
...
The stable architecture now supports explicit register/update/remove of ordinary filesystem/project roots without another MCP schema publication. Per-workspace capability checks remain server-owned; semantic Git selects only `workspaceId`, derives branch/upstream dynamically, preserves the registered remote and integrity policy, and exposes no caller-selected cwd/URL/refspec/credentials/config/profile/sandbox/force inputs. Authenticated private Git is proven through the bounded host-network substrate. The latest preserved local-runtime boundary is `e96d53248de6718d14072dd857325a15587be5dc`. It preserves the large-PDF page-render candidate, real Machine Learning page-16 feasibility evidence, guarded installed-source publication and rollback metadata while retaining the earlier host-capability, Astra Phase 2 and GPT-5.6 Sol Browser evidence in private history. The latest private push passed `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS`, exact local/remote equality, clean tracked postflight, and `postflightOk=true`.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `large-pdf-page-render-source-published-restart-pending` boundary unless the repository has advanced further. The narrow renderer-only candidate is already source-published and statically qualified; the exact next step is the controlled Codexless/tunnel restart from the governing operations runbook followed by same-chat `codex.document_render` qualification of `51.Deep Learning2.annotated.pdf` page 16. No Plugin refresh is required because the public tool schema and tool count did not change. For operational continuation steps, the collaborator must also resolve and read the governing procedure referenced by the active boundary before giving exact execution instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
