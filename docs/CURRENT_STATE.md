...
Checkpoint 318 is the current boundary. Research 119 remains the governing scope correction, and Research 120 governs the automatic hybrid PDF direct-source architecture. The 192 MiB direct `document_read` / `document_render` envelope is now live-qualified after restart on a valid 134,218,426-byte PDF. The Research 120 private core is also end-to-end qualified: source profiling, deterministic `pypdf 6.10.0` native splitting, page/range isolation above the direct-processing envelope, a Codexless-owned content-addressed artifact cache with source-authority revalidation, cached embedded-text and rendered-page adapters, cached PDF resource delivery, and the model-free `PdfAccessOrchestrator` all pass. The next implementation seam is no longer the artifact manager/splitter; it is the bounded public high-level MCP facade and its content/resource/image projection.
...
The private local-runtime repository is synchronized exactly to `origin/main` at `3cf995e358425c4db970337f0413a8862bb5a564` with `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS` and `postflightOk=true`. It now preserves the qualified Research 120 hybrid-routing core, including the source profiler, deterministic splitter/isolation path, managed artifact cache, cached text/render/resource adapters and high-level orchestrator, in addition to the earlier 192 MiB source-processing, large-PDF text/render, host-capability, Astra and Browser history.

The deterministic multi-native-PDF `document_file_link` qualification remains a real fallback, and both first-class large-source fallback channels remain fully qualified. Research 120 now defines and implements their automatic route selection and derived-artifact lifecycle. The combined private core passes 42/42 tests, including cache restart reuse, deterministic regeneration after eviction, atomic failed-generation cleanup, source-drift invalidation, native split render equivalence, cached embedded text, cached page vision, cached PDF resources and a valid >192 MiB page-isolation text/render path. Real read-only planning on `51.Deep Learning2.annotated.pdf` produces bounded native parts with oversized pages 16 and 49 routed to text/render fallback. The public 192 MiB `document_read` and `document_render` envelope is already live-qualified after the completed restart. The next implementation seam is the public high-level MCP facade.
...
The stable architecture now supports explicit register/update/remove of ordinary filesystem/project roots without another MCP schema publication. Per-workspace capability checks remain server-owned; semantic Git selects only `workspaceId`, derives branch/upstream dynamically, preserves the registered remote and integrity policy, and exposes no caller-selected cwd/URL/refspec/credentials/config/profile/sandbox/force inputs. Authenticated private Git is proven through the bounded host-network substrate. The latest preserved local-runtime boundary is `3cf995e358425c4db970337f0413a8862bb5a564`. It preserves the qualified Research 120 hybrid-routing core, the 192 MiB direct-processing source candidate and the completed large-PDF text/render qualifications while retaining earlier host-capability, Astra Phase 2 and GPT-5.6 Sol Browser evidence in private history. The latest private push passed `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS`, exact local/remote equality, clean tracked postflight, and `postflightOk=true`.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `hybrid-pdf-routing-core-qualified-public-facade-next` boundary unless the repository has advanced further. Research 120 is the governing automatic PDF architecture. Do not repeat the 192 MiB restart qualification, artifact-manager/splitter/isolation implementation, or the 42/42 private core regression. The exact next implementation seam is the bounded public high-level PDF access facade over the qualified `PdfAccessOrchestrator`, including MCP resource-link projection for native whole/part PDFs, text structured-content projection, image projection for rendered pages, full existing-surface regression, guarded publication and the normal tool-projection refresh/fresh-chat handling required by AB-008. For operational continuation steps, the collaborator must also resolve and read the governing procedure referenced by the active boundary before giving exact execution instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
