...
Checkpoint 329 is the current boundary. Research 120 is qualified and closed for the accepted PDF direct-source routing scope. Research 121 now governs the non-PDF Office capability matrix. Current OpenAI product documentation supports DOCX, PPTX and XLSX as ordinary ChatGPT upload types, while current MCP `resource_link` / `resources/read` semantics are MIME-generic rather than PDF-specific. Local source inspection also shows that the existing PDF whole-file resource lifecycle already owns the reusable authority, canonical-containment, size/hash/identity binding, expiring opaque-token and read-time revalidation machinery; its PDF restriction is concentrated in media/header validation. The accepted next experiment is therefore one narrow `codex.file_link` candidate for allowlisted OOXML files rather than three custom parsers. The candidate must preserve existing PDF contracts, use existing workspace read authority only, derive MIME server-side, reject path escapes and unsupported types, bind exact source identity/content, return no inline file bytes, and expose bytes only through an expiring server-owned resource. Tiny deterministic DOCX/PPTX/XLSX fixtures will then be used in a fresh ChatGPT host to distinguish materialization, text/data/structure access, visual/layout fidelity and whether any format-specific fallback is actually needed.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `non-pdf-office-handoff-reuse-first-candidate-next` boundary unless the repository has advanced further. Research 120 is complete for its accepted automatic PDF architecture scope; Research 121 governs the current Office whole-file handoff work. Do not reopen native split/native hybrid, direct text/render/mixed, auto-routing, renderer-repair, or >192 MiB PDF qualifications without new evidence. The exact next implementation step is a private `codex.file_link` candidate for only DOCX, PPTX and XLSX, reusing the existing resource lifecycle while leaving PDF tools/contracts unchanged. Qualify workspace-read authority, canonical containment, extension/MIME allowlisting, OOXML ZIP signature, size/hash/identity binding, opaque TTL resources, read-time drift detection, no-inline-bytes behavior and rejection of unsupported types; then generate tiny deterministic fixtures for staged and fresh-host materialization/fidelity testing. Preserve the `--allow-addons` warning observed during cached PDF rendering as a hardening caveat: the trusted native canvas addon is accepted for current functionality, but the Node permission model is not an OS-grade isolation guarantee. AB-008 remains relevant to fresh-chat schema acquisition. AB-020 remains relevant because the private integrity gate reached its 32 KiB tracked-path enumeration edge during Checkpoint 323 preservation; do not weaken that gate merely to add candidate files. For operational continuation steps, the collaborator must resolve and read the governing procedure referenced by the active boundary before giving exact restart/publication instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
