...
Checkpoint 308 is the current boundary. Research 119 preserves the project-owner correction that the current file/document work is intended to give ordinary ChatGPT chat direct bounded access to authorized files on the user's local machine through Codexless. The target is not the future ADS product's document architecture and is not a Codex semantic-worker architecture. The ADS repository is the development/preservation vehicle for this capability; eventual ADS product reuse remains a separate design decision.

The manual-upload baseline matters: the user can already attach supported files, including large PDFs, directly to ChatGPT. Therefore a pipeline where Codex reads a local file and explains it to ChatGPT is not a solution to the current access problem. The desired improvement is that ChatGPT can obtain the actual local file or a faithful model-free source representation automatically from an already-authorized workspace, without manual upload and without requiring another reasoning model merely because the file is local.

Existing direct-access results remain accepted. `codex.read_many` provides bounded local text. `codex.image_read` sends authorized local PNG/JPEG/WebP content directly into ChatGPT vision. `codex.document_read` sends deterministic PDF text directly to ChatGPT. `codex.document_render` sends rendered PDF pages directly to ChatGPT vision within its qualified ordinary-page envelope. `codex.document_file_link` can materialize the actual local PDF into the ChatGPT conversation and enable normal next-turn PDF inspection within the clean host-qualified range. The clean whole-PDF interval remains 7,417,428 bytes PASS to 7,993,210 bytes FAIL, without claiming a universal threshold.

Research 118 and the private Astra Phase 2 semantic-evidence candidate remain preserved as historical/optional delegated-document-analysis research. Their useful transport, Browser, runtime, source-hashing and host-boundary findings are retained. The semantic worker/receipt is no longer the active Research 117 solution because it inserts a reasoning-model intermediary instead of giving ChatGPT direct source access. The prepared GPT-6 Astra Attempt 02 was explicitly declined before any model turn started, so no second semantic qualification is pending.

The exact next Research 117/119 action is to build and test a direct ChatGPT local-file capability matrix by file type. Priority starts with extending or complementing large-PDF access beyond the current `resource_link` host-materialization boundary without a reasoning-model intermediary, then testing direct/native whole-file handoff and faithful model-free fallbacks for DOCX, PPTX, XLSX and other practically relevant file families. Browser upload remains only a possible transport fallback if cleaner supported host/file mechanisms are inadequate.
