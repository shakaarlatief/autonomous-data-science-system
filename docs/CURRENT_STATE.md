...
Checkpoint 328 is the current boundary. Research 119 remains the governing scope correction. Research 120 is now qualified for the accepted automatic hybrid PDF direct-source scope. The final previously unqualified public route family passed on a valid authorized 211,813,221-byte synthetic PDF, above the 201,326,592-byte direct-processing ceiling. Explicit text selected `isolate-then-text`, explicit visual selected `isolate-then-render`, and explicit mixed selected `isolate-then-text-plus-render`. The first call generated one 862-byte managed page-isolation artifact; the later visual and mixed calls reused the same generation key, generation id and artifact SHA-256. Both text and image outputs preserved `sourcePageNumber:1`; the image was directly inspectable in ChatGPT and remained identical between visual and mixed calls. The original source hash was unchanged after all calls, and its source directory still contained only the original PDF, so no derived file was written beside the source. Together with the native, direct and auto-route qualifications at Checkpoints 320, 321 and 327, the complete accepted public PDF route family is now qualified. The broader AB-005 document work can advance to the next file-type capability matrix rather than adding more PDF size-threshold tests without new evidence.
