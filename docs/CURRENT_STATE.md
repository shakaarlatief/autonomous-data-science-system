...
Checkpoint 336 is the current boundary. Research 120 remains qualified and closed for the accepted PDF direct-source routing scope, while Research 121 governs the Office whole-file handoff work. AB-020 remains live-qualified and closed. The second ordinary-host preview.17 `-Publish` attempt passed all staged preflight regressions and reached host confirmation, but the first existing-file replacement failed because Windows PowerShell/.NET rejected `File.Replace` when the helper passed a null backup-file argument. The helper caught the failure, and independent read-only verification proved exact rollback to preview.16: all six existing package targets match their original hashes, all three new Office files are absent, the AB-020 semantic-Git hash is unchanged, and the running surface remains preview.16 / 60 tools. The helper now uses a real timestamped backup path in the atomic `File.Replace` call, uses a verified rollback temp plus displaced-file backup for atomic rollback, and exercises the exact replacement primitive in a bounded preflight smoke test. The corrected helper hashes to `05839717f1b09360c514d86985180b57dd029b088b1b09a0bdc129f9b54a5ee5` and has passed two consecutive full no-publish preflights including `OFFICE_FILE_REPLACE_PRIMITIVE=PASS`. The exact next action is retrying ordinary-host preview.17 publication, then independent installed-hash verification, controlled restart, and fresh ChatGPT discovery before the DOCX/PPTX/XLSX host fidelity matrix.
