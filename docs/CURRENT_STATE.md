...
Checkpoint 312 is the current boundary. Research 119 remains the governing scope correction: ordinary ChatGPT should gain direct bounded access to authorized local-machine files through Codexless without requiring manual upload or a reasoning-model intermediary. Checkpoint 311's multi-native-PDF fallback remains accepted, but a generalization check against the larger read-only Machine Learning PDFs shows that native page-range splitting alone is not sufficient: several files contain individual pages whose one-page native PDF representations are themselves larger than the host-qualified envelope. The direct-access design therefore needs a hybrid oversized-page fallback rather than immediate native-split-only productization.
...
The deterministic multi-native-PDF `document_file_link` qualification remains a real fallback, but Validation 070 prevents treating it as universal. Fifteen Machine Learning PDFs exceed the clean whole-file PASS, and several contain individual pages above a 7,000,000-byte planning target, including a 17,597,874-byte page in `11.Introduction.annotated.pdf` and a 15,944,609-byte page in `51.Deep Learning2.annotated.pdf`. Basic pypdf compression/deduplication does not materially reduce the worst case. The next Research 117/119 action is therefore to qualify a hybrid oversized-single-page route that keeps source workspaces read-only and gives ChatGPT faithful model-free page text plus visual/source representation without another reasoning model.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `machine-learning-large-pdf-hybrid-fallback-required` boundary unless the repository has advanced further. The deterministic multi-native-PDF direct-access experiment remains PASS evidence, but the Machine Learning generalization check shows that a native page-range splitter alone cannot handle files containing individual pages larger than the host envelope. The next bounded research step is to qualify a model-free oversized-single-page route without requiring source-workspace write authority. For operational continuation steps, the collaborator must also resolve and read the governing procedure referenced by the active boundary before giving exact execution instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
