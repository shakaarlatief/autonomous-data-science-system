...
Checkpoint 319 is the current boundary. Research 119 remains the governing scope correction, and Research 120 governs the automatic hybrid PDF direct-source architecture. The 192 MiB direct `document_read` / `document_render` envelope is live-qualified after restart on a valid 134,218,426-byte PDF. The Research 120 model-free core and bounded `codex.pdf_access` public facade are now source-qualified: source profiling, deterministic `pypdf 6.10.0` native splitting, page/range isolation above the direct-processing envelope, a Codexless-owned content-addressed artifact cache with source-authority revalidation, cached embedded-text/render/resource adapters, the high-level `PdfAccessOrchestrator`, strict MCP text/resource/image projection, native-result bounding and cache-pruning policy all pass. The combined private suite is 49/49 PASS and the complete staged public regression passes at the intended 60-tool candidate surface. The exact qualified source/test payload has been guardedly published to the installed Codexless tree and independently hash-verified, but the active process intentionally remains on preview.15 / 59 tools until the controlled restart. The next seam is activation and fresh-host qualification, not more core implementation.
...
The currently running Codexless server remains `0.1.1-preview.15-host-capability-probe` with 59 source tools until the controlled restart. Its tunnel remains healthy and ready. The installed source/test tree, however, now contains the exact qualified `0.1.1-preview.16-hybrid-pdf-access` / 60-tool candidate payload, including `codex.pdf_access`; independent post-publication hashes match all 17 candidate source files and four adapted installed regression files. Do not claim the 60-tool surface is live until restart health confirms it. The existing 96 MiB resource-link source ceiling, near-threshold host materialization evidence, native `auto_review`, and direct-Codex guard remain otherwise unchanged.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `hybrid-pdf-access-live-source-published-restart-pending` boundary unless the repository has advanced further. Research 120 is the governing automatic PDF architecture. Do not repeat the 192 MiB restart qualification, artifact-manager/splitter/isolation implementation, 49/49 private core/facade regression, staged 60-tool public-surface regression, or guarded source publication. The exact next action is operational activation, not further implementation: read and follow `docs/local_execution/OPERATIONS.md`, perform the controlled Codexless/tunnel restart sequence, verify preview.16 / toolCount 60 and tunnel readiness, refresh the existing developer MCP app, then use a fresh disposable chat to discover and qualify `codex.pdf_access` end to end. AB-008 remains relevant because the current persistent conversation has previously retained stale callable projections after refresh. For operational continuation steps, the collaborator must resolve and read the governing procedure referenced by the active boundary before giving exact execution instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
