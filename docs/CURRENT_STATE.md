...
Checkpoint 309 is the current boundary. Research 119 remains the governing scope correction: ordinary ChatGPT should gain direct bounded access to authorized local-machine files through Codexless without requiring manual upload or a reasoning-model intermediary. This capability is being developed in the ADS repositories but remains distinct from future ADS product document architecture.
...
Checkpoint 309 adds one narrowly scoped direct-host experiment before PDF splitting. Current MCP Apps protocol evidence allows a host to advertise `updateModelContext` with a `resourceLink` modality. OpenAI's current Plugin documentation also acknowledges files returned by tool file references, but no general custom-MCP mechanism for minting a ChatGPT file ID from arbitrary local bytes has been established. To avoid guessing, a temporary read-only MCP Apps host-capability probe was implemented and preserved privately, then live-published as Codexless `0.1.1-preview.15-host-capability-probe` with 59 source tools. Direct `/healthz` verification reports that exact version/tool count and the Secure MCP Tunnel `/readyz` reports `ready`.

The user refreshed `ADS Codexless Local Bridge` in this existing persistent conversation after the controlled restart. ChatGPT-side tool rediscovery nevertheless retained the older callable projection and did not expose the three newly added probe tools. This is another exact reproduction of the known same-conversation projection-staleness behavior from Validation 034 / AB-008, not evidence that the live 59-tool server failed to publish the tools.

The temporary exact-root `codexless-live` ordinary workspace admission used only for one-time publication was removed after verification. The durable registry is again revision 7 / content hash `5cd85c57465e7f3da0a19888f28c8e0afe128dbb32f7ff2f7056bd6021dbaa31` with only `ads-public`, `ads-local-runtime`, `big-data-statistics`, and `machine-learning`.

The host-capability candidate, live-publication evidence, resolved public-preservation handoff, and exact guarded Checkpoint 309 public Git-ACL repair helper are preserved in the private runtime repository at `5f60d2e059bdef9b3c3e268f6e6507d65e1a3dac`, synchronized exactly to `origin/main` with `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS` and `postflightOk=true`. The current private candidate tree contains only the four source delta files plus its tests/docs; the full qualification snapshot remains in its parent history.

The exact next Research 117/119 action is therefore a fresh disposable ChatGPT conversation using the refreshed Plugin. That fresh conversation must call `codex.host_capability_probe`, let the MCP App complete `ui/initialize`, then call `codex.host_capability_probe_result` and classify `updateModelContext.resourceLink` as `ADVERTISED` or `NOT_ADVERTISED`. A positive advertisement permits only a tiny direct PDF resource-link model-context qualification; it does not establish large-file success. A negative result closes this route for the current host and returns the workstream to the deterministic multi-native-PDF `document_file_link` experiment. Browser upload remains only a later transport fallback.
...
The workspace registry is now revision `7`, content hash `5cd85c57465e7f3da0a19888f28c8e0afe128dbb32f7ff2f7056bd6021dbaa31`, with `ads-public`, `ads-local-runtime`, `big-data-statistics`, and `machine-learning`. A temporary exact-root `codexless-live` read/write admission was created only for the Checkpoint 309 one-time publication after the existing runtime-maintenance authority gap blocked publication from `ads-local-runtime`, then removed again immediately after live verification. `big-data-statistics` has only `read` and `agent`; `machine-learning` remains strictly `read` only. Neither personal workspace has write, browser, or Git capability. Exact personal roots remain machine-local rather than public project authority.
...
The stable architecture now supports explicit register/update/remove of ordinary filesystem/project roots without another MCP schema publication. Per-workspace capability checks remain server-owned; semantic Git selects only `workspaceId`, derives branch/upstream dynamically, preserves the registered remote and integrity policy, and exposes no caller-selected cwd/URL/refspec/credentials/config/profile/sandbox/force inputs. Authenticated private Git is proven through the bounded host-network substrate. The latest preserved local-runtime boundary is `5f60d2e059bdef9b3c3e268f6e6507d65e1a3dac`. It preserves the live-published ChatGPT host-capability probe candidate and qualification evidence while retaining the earlier Astra Phase 2 source-bound PDF evidence candidate and frozen GPT-5.6 Sol Browser baseline in private history. The latest private push passed `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS`, `retried=false`, exact local/remote equality, clean tracked postflight, and `postflightOk=true`.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `chatgpt-host-capability-probe-live-fresh-chat-required` boundary unless the repository has advanced further. For the immediate disposable qualification chat, the required first action is narrower: verify the refreshed Plugin exposes the host-capability tools, run the read-only probe, and preserve its result back into the authoritative repository before broader continuation. For operational continuation steps, the collaborator must also resolve and read the governing procedure referenced by the active boundary before giving exact execution instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
