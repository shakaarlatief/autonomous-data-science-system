...
Checkpoint 332 is the current boundary. Research 120 remains qualified and closed for the accepted PDF direct-source routing scope, while Research 121 governs the non-PDF Office capability matrix. Checkpoint 331 preserves the already-qualified `codex.file_link` Office scratch candidate and deterministic DOCX/PPTX/XLSX host fixtures. The AB-020 bounded private-integrity correction is now published into the installed Codexless source tree and independently re-read at SHA-256 `3b2ddbbe00339045b81044bb3e1e39c314a461a7e0f8e4e608b79b4b0f37de02`. The publication helper reran the 8/8 focused private suite and three live-disk public regressions and reported PASS, with a timestamped backup and `RESTART_PERFORMED=false`. A separate read-only probe confirmed the installed hash exactly, while the currently running preview.16 / 60-tool process and tunnel remain healthy at HTTP 200/200. Because the process has not yet been restarted, AB-020 is source-published but not live-qualified. The exact next step is the full controlled restart under `docs/local_execution/OPERATIONS.md`, followed by one real private semantic Git push with tracked-path enumeration above the old 32 KiB envelope. Only after that live proof should the Office candidate move from protected scratch into durable private Git.
...
The private local-runtime repository is synchronized exactly to `origin/main` at `7e70bd05e4da76ff1ad260b2b1cbdc5c1d65a3fb` with `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS` and `postflightOk=true`. It now preserves the bounded AB-020 integrity-enumeration candidate in addition to the qualified Research 120 hybrid-routing core, the bounded public `codex.pdf_access` facade, the live preview.16 publication/native-host qualifications, and earlier Astra/Browser/runtime evidence. The original 32 KiB failure remains preserved as evidence, but the candidate replaces the full-list outer transport with a compact scanner receipt while keeping the generic command-output cap unchanged. The private push itself still passed under the old live integrity implementation because no new tracked path had yet been added. AB-020 is therefore candidate/preflight-qualified but not closed until the corrected source is published, restarted, and a real semantic push succeeds with tracked-path enumeration above 32 KiB.
...
The stable architecture now supports explicit register/update/remove of ordinary filesystem/project roots without another MCP schema publication. Per-workspace capability checks remain server-owned; semantic Git selects only `workspaceId`, derives branch/upstream dynamically, preserves the registered remote and integrity policy, and exposes no caller-selected cwd/URL/refspec/credentials/config/profile/sandbox/force inputs. Authenticated private Git is proven through the bounded host-network substrate. The latest preserved local-runtime boundary is `ad61a5619165ec5675e75daecdb4fdb29ea6f19a`. It preserves the qualified Research 120 hybrid-routing core, the bounded `codex.pdf_access` facade, the guarded live-source publication/native host evidence, the 192 MiB direct-processing source path, and the new renderer-serialization candidate with 51/51 regression coverage while retaining earlier host-capability, Astra Phase 2 and GPT-5.6 Sol Browser evidence in private history. The latest private push passed `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS`, exact local/remote equality, clean tracked postflight, and `postflightOk=true`.
...
The live Codexless server is now `0.1.1-preview.16-hybrid-pdf-access` with 60 source tools and `codexless-public-preview-v2`; tunnel `/healthz` and `/readyz` both return HTTP 200. Fresh disposable ChatGPT conversations have qualified the native split, native hybrid and five-call semantic intent routes end to end. This current persistent conversation now also exposes `codex.pdf_access` as a callable action; the earlier same-chat stale-projection observations remain preserved as historical AB-008 evidence rather than a current callable-state claim. The existing 96 MiB resource-link source ceiling, near-threshold host materialization evidence, native `auto_review`, and direct-Codex guard remain otherwise unchanged.
...
The guided handoff boundary is preserved through Checkpoint 275. `chatgpt-17` then continued Research 113/117 through the document-resource and Browser-baseline work and closed at Checkpoint 305 after its own context-limit termination. `chatgpt-18` completed the independent Astra architecture challenge, corrected the workstream objective, qualified the host-capability, native split, large-source text/render and complete native-hybrid PDF routes, and closed at Checkpoint 321 when the conversation reached its length limit. The current `chatgpt-19` interaction preserved the failed fresh-chat intent matrix as Checkpoint 322, localized its rendering-dependent failures to batched direct-render transport across the already-known Windows App Server buffered stdout ceiling, qualified the bounded per-page serialization correction as Checkpoint 323 with 51/51 private tests and exact private preservation at `ad61a5619165ec5675e75daecdb4fdb29ea6f19a`, qualified the exact guarded live-publication package as Checkpoint 324 after 51/51 private and 7/7 staged public regressions, completed host publication plus independent installed-hash verification as Checkpoint 325, live-qualified the repaired low-level renderer after controlled restart as Checkpoint 326, closed the fresh disposable five-call high-level intent matrix as Checkpoint 327, qualified the >192 MiB public text/render/mixed isolation routes with managed-artifact reuse as Checkpoint 328, and opened Research 121's reuse-first Office handoff work as Checkpoint 329. The active boundary remains direct ChatGPT local-machine file access through Codexless. The exact next implementation step is the narrow private `codex.file_link` candidate for allowlisted DOCX/PPTX/XLSX whole-file resource handoff.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `ab-live-source-published-restart-pending` boundary unless the repository has advanced further. Research 120 is complete for its accepted automatic PDF architecture scope; Research 121 governs the current Office whole-file handoff work. The exact next action is the full controlled restart sequence in `docs/local_execution/OPERATIONS.md`. After restart, verify local preview.16 / 60-tool health and tunnel HTTP 200/200, then prove one real private semantic Git commit/push while the tracked-path enumeration is above the old 32 KiB outer-response ceiling. Only after that live proof should the already-qualified `codex.file_link` source/tests and DOCX/PPTX/XLSX fixtures be moved from protected scratch into durable private Git. Do not widen the generic `command/exec` output cap or weaken private secret/binary/content gates. Preserve the `--allow-addons` warning observed during cached PDF rendering as a hardening caveat. AB-008 remains relevant to fresh-chat schema acquisition. For operational continuation steps, the collaborator must resolve and read the governing procedure referenced by the active boundary before giving exact restart/publication instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
