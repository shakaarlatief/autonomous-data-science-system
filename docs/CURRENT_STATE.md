...
Checkpoint 325 is the current boundary. Research 119 remains the governing scope correction, and Research 120 governs the automatic hybrid PDF direct-source architecture. The qualified host publication has now completed successfully: the installed `document-renderer.mjs` independently hashes to `42199fca624f931f0076a502dbe4c4710f26f0769db50a64a2ac2174b6899b43` and the matching installed `document-render-regression.mjs` independently hashes to `3e60f761ebf5d68dcf4b05969e62dc3cf3d6931aebe4ab7403b66fc37ac4b725`. The publication helper reran the complete seven-script live-disk regression set at the 60-tool preview.16 surface and reported PASS, with backups preserved for both replaced files and `RESTART_PERFORMED=false`. A separate read-only probe then re-read both installed hashes and confirmed that the running process and tunnel still report `0.1.1-preview.16-hybrid-pdf-access`, 60 tools, `codexless-public-preview-v2`, and tunnel health/readiness HTTP 200. Because the process has not been restarted, the new renderer bytes are source-published but not yet live-qualified behavior. `docs/local_execution/OPERATIONS.md` was re-read and governs the exact restart order. The next legitimate step is the full controlled restart, then a fresh disposable intent-matrix retest before the >192 MiB facade-isolation proof.
