...
Checkpoint 321 is the current boundary. Research 119 remains the governing scope correction, and Research 120 governs the automatic hybrid PDF direct-source architecture. The live Codexless runtime remains `0.1.1-preview.16-hybrid-pdf-access` / 60 tools with tunnel health/readiness HTTP 200. A fresh disposable ChatGPT conversation executed exactly one `codex.pdf_access` call on the 78,874,939-byte `51.Deep Learning2.annotated.pdf` with `intent=native`. The facade selected `native-parts-plus-page-fallback`, reused fourteen deterministic managed native PDF parts for all ordinary pages, and routed individually oversized source pages 16 and 49 to direct embedded-text plus rendered-image fallback. Both returned PNGs were inspected directly from the same facade call, all fourteen native PDF resource links materialized as actual ChatGPT conversation files, and ordinary ChatGPT-side PDF inspection succeeded on one materialized part. This closes the high-level `intent=native` hybrid route end to end at the host boundary. Remaining Research 120 public qualification should now target the explicit `text`, `visual`, `mixed`, and `auto` intent matrix plus the >192 MiB page/range-isolation route rather than repeating native routing.
...
The guided handoff boundary is preserved through Checkpoint 275. `chatgpt-17` then continued Research 113/117 through the document-resource and Browser-baseline work and closed at Checkpoint 305 after its own context-limit termination. The current `chatgpt-18` interaction completed the independent Astra architecture challenge, exposed the frozen Sol implementation only after Phase 1, reconciled and preserved the Phase 2 candidate at Checkpoint 306, preserved the first held-out worker as `AMBIGUOUS` at Checkpoint 307, corrected the workstream objective at Checkpoint 308, and live-published the narrow ChatGPT host-capability diagnostic at Checkpoint 309. The active boundary remains direct ChatGPT local-machine file access through Codexless; the next qualification must occur in a fresh disposable ChatGPT conversation because this originating conversation retained a stale callable-tool projection after Plugin refresh.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `hybrid-pdf-access-fresh-chat-native-hybrid-qualified` boundary unless the repository has advanced further. Research 120 is the governing automatic PDF architecture. Do not repeat the 192 MiB qualification, artifact-manager/splitter/isolation implementation, 49/49 private core/facade regression, staged 60-tool public-surface regression, guarded publication, controlled restart, developer-app refresh, the fresh-chat native split qualification, or the fresh-chat native hybrid qualification. The exact next Research 120 work is the remaining public intent matrix and isolation proof: explicit `text`, `visual`, `mixed`, and `auto` facade routes, followed by a >192 MiB page/range-isolation qualification through the same high-level facade. AB-008 remains relevant because this persistent conversation still retains an older callable projection while fresh disposable chats discover the new facade correctly. For operational continuation steps, the collaborator must resolve and read the governing procedure referenced by the active boundary before giving exact execution instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
