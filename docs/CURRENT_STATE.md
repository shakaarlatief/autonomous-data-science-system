...
The private local-runtime repository is synchronized exactly to `origin/main` at `8e2b98cfe5cd7eecafb9764dfd9e29b5602638d4` with `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS` and `postflightOk=true`. It now preserves both the qualified Research 120 hybrid-routing core and the bounded public `codex.pdf_access` facade, including the source profiler, deterministic splitter/isolation path, managed artifact cache, cached text/render/resource adapters, high-level orchestrator, MCP projection layer, 60-tool candidate surface and live-source publication evidence, in addition to the earlier 192 MiB source-processing, large-PDF text/render, host-capability, Astra and Browser history.
