...
Checkpoint 322 is the current boundary. Research 119 remains the governing scope correction, and Research 120 governs the automatic hybrid PDF direct-source architecture. The live Codexless runtime remains `0.1.1-preview.16-hybrid-pdf-access` / 60 tools with tunnel health/readiness HTTP 200. The fresh disposable five-call `codex.pdf_access` intent matrix failed overall: explicit `text` and `auto` with `visualRequired=false` both qualified as `direct-text`, while `visual`, `mixed`, and `auto` with `visualRequired=true` failed at the lower renderer with `DOCUMENT_RENDER_PROTOCOL_ERROR: sandboxed renderer returned invalid JSON`. Call 2 also contained a preserved input deviation because `visualPages:[16,49]` was supplied in addition to the scheduled `pages:[16,49]`; Calls 3 and 5 matched their scheduled inputs and independently reproduced the same failure. Persistent-chat low-level discrimination then reproduced failure for `codex.document_render` pages `[16,49]` while pages 16 and 49 each passed individually with the same PNG hashes qualified at Checkpoint 321. This localizes the active defect to multi-page direct-render transport through the previously documented Windows buffered `command/exec` stdout ceiling, not to the intent policy or page fidelity. The next legitimate step is the smallest renderer correction: serialize selected direct-source pages as bounded one-page sandbox executions, combine/validate them under the existing public page and aggregate limits, regress the candidate, publish/restart only after qualification, then rerun the failed intent matrix before proceeding to the >192 MiB facade isolation proof.
...
The private local-runtime repository is synchronized exactly to `origin/main` at `d1728207a5b0a4e3f168fad999aa400c2db0019d` with `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS` and `postflightOk=true`. It now preserves both the qualified Research 120 hybrid-routing core and the bounded public `codex.pdf_access` facade, including the source profiler, deterministic splitter/isolation path, managed artifact cache, cached text/render/resource adapters, high-level orchestrator, MCP projection layer, 60-tool candidate surface, live-source publication evidence, fresh-host native split qualification and fresh-host native hybrid qualification, in addition to the earlier 192 MiB source-processing, large-PDF text/render, host-capability, Astra and Browser history.

The deterministic multi-native-PDF `document_file_link` qualification remains a real fallback, and both first-class large-source fallback channels remain fully qualified. Research 120 now defines and implements their automatic route selection and derived-artifact lifecycle. The combined private core/facade suite passes 49/49 tests, including cache restart reuse, deterministic regeneration after eviction, atomic failed-generation cleanup, source-drift invalidation, native split render equivalence, cached embedded text, cached page vision, cached PDF resources, >192 MiB page-isolation text/render, MCP facade projection, native-result bounding and managed-cache pruning. Real read-only planning on `51.Deep Learning2.annotated.pdf` produces bounded native parts with oversized pages 16 and 49 routed to text/render fallback. The public 192 MiB `document_read` and `document_render` envelope is already live-qualified. The `codex.pdf_access` source payload is now installed but not yet active in the running process; controlled restart and fresh-host qualification are next.
...
The stable architecture now supports explicit register/update/remove of ordinary filesystem/project roots without another MCP schema publication. Per-workspace capability checks remain server-owned; semantic Git selects only `workspaceId`, derives branch/upstream dynamically, preserves the registered remote and integrity policy, and exposes no caller-selected cwd/URL/refspec/credentials/config/profile/sandbox/force inputs. Authenticated private Git is proven through the bounded host-network substrate. The latest preserved local-runtime boundary is `d1728207a5b0a4e3f168fad999aa400c2db0019d`. It preserves the qualified Research 120 hybrid-routing core, the bounded `codex.pdf_access` facade, the guarded live-source publication evidence, the fresh-host native split and native hybrid qualifications, the 192 MiB direct-processing source path and the completed large-PDF text/render qualifications while retaining earlier host-capability, Astra Phase 2 and GPT-5.6 Sol Browser evidence in private history. The latest private push passed `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS`, exact local/remote equality, clean tracked postflight, and `postflightOk=true`.
...
The live Codexless server is now `0.1.1-preview.16-hybrid-pdf-access` with 60 source tools and `codexless-public-preview-v2`; tunnel `/healthz` and `/readyz` both return HTTP 200. A fresh disposable ChatGPT conversation after developer-app refresh discovered `codex.pdf_access` and qualified the native split route end to end, including host materialization of two managed PDF resource links. The current long-running persistent conversation still does not expose the new facade as a callable action, reinforcing AB-008's stale-projection behavior without weakening the fresh-chat qualification. The existing 96 MiB resource-link source ceiling, near-threshold host materialization evidence, native `auto_review`, and direct-Codex guard remain otherwise unchanged.
...
The guided handoff boundary is preserved through Checkpoint 275. `chatgpt-17` then continued Research 113/117 through the document-resource and Browser-baseline work and closed at Checkpoint 305 after its own context-limit termination. `chatgpt-18` completed the independent Astra architecture challenge, corrected the workstream objective, qualified the host-capability, native split, large-source text/render and complete native-hybrid PDF routes, and closed at Checkpoint 321 when the conversation reached its length limit. The current `chatgpt-19` interaction preserved the failed fresh-chat intent matrix as Checkpoint 322 and localized its rendering-dependent failures to batched direct-render transport across the already-known Windows App Server buffered stdout ceiling. The active boundary remains direct ChatGPT local-machine file access through Codexless. The next implementation step is the bounded multi-page direct-render serialization correction, followed by candidate regression, guarded publication/restart under the governing runbook, and a fresh disposable intent-matrix retest.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `hybrid-pdf-intent-matrix-render-transport-gap` boundary unless the repository has advanced further. Research 120 is the governing automatic PDF architecture. Do not repeat the native split or native hybrid qualifications, the lower-level 192 MiB primitives, or the failed five-call intent matrix merely as a substitute for repairing the observed defect. The exact next Research 120 work is to serialize direct-source multi-page rendering internally into bounded one-page sandbox executions, preserve the existing public four-page request contract and aggregate limits, regress and publish that correction, and then rerun the intent matrix in a fresh disposable chat. Only after the rendering-dependent branches pass should the public >192 MiB page/range-isolation qualification proceed through the same high-level facade. AB-008 remains relevant to fresh-chat schema acquisition. For operational continuation steps, the collaborator must resolve and read the governing procedure referenced by the active boundary before giving exact restart/publication instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
