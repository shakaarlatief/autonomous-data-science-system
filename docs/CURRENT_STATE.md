...
Checkpoint 327 is the current boundary. Research 119 remains the governing scope correction, and Research 120 governs the automatic hybrid PDF direct-source architecture. The fresh disposable five-call `codex.pdf_access` intent matrix now passes completely after the serialized-page renderer fix. Explicit `text` resolved to `direct-text`; explicit `visual` resolved to `direct-render`; explicit `mixed` resolved to `direct-text-plus-selective-render`; `auto` with `visualRequired=false` resolved to text/direct-text; and `auto` with `visualRequired=true` resolved to mixed/direct-text-plus-selective-render. All rendering-dependent calls returned both source pages 16 and 49 with the same live PNG hashes qualified at Checkpoints 321 and 326. Text routes returned the same untruncated 19,344 characters across pages 16 and 49. Every image-bearing call was directly inspected in ChatGPT, all five calls reported zero PDF resource links, and no alternate ADS tool, OCR, Browser, Agent, source-workspace write, manual source upload or ordinary ChatGPT PDF/file tooling was used. This closes the renderer/intention-matrix repair sequence opened at Checkpoint 322. The exact next Research 120 work is now the public `codex.pdf_access` page/range-isolation proof for a source above the 192 MiB direct-processing envelope.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `hybrid-pdf-intent-matrix-qualified-isolation-next` boundary unless the repository has advanced further. Research 120 is the governing automatic PDF architecture. Do not repeat the native split/native hybrid qualifications, the lower-level 192 MiB primitives, the failed pre-fix intent matrix, the serialization implementation, publication/restart work, or the now-passing five-call intent matrix as substitutes for advancing the boundary. The exact next Research 120 work is the public `codex.pdf_access` page/range-isolation qualification on a valid source above 192 MiB. That proof should exercise the `isolate-then-text`, `isolate-then-render`, and preferably `isolate-then-text-plus-render` route classes with direct ChatGPT text/image consumption, original-source page provenance, no source-workspace writes, and managed-artifact reuse where observable. AB-008 remains relevant to fresh-chat schema acquisition. AB-020 remains relevant because the private integrity gate reached its 32 KiB tracked-path enumeration edge during Checkpoint 323 preservation. For operational continuation steps, the collaborator must resolve and read the governing procedure referenced by the active boundary before giving exact restart/publication instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
