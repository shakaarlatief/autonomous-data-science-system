...
Checkpoint 316 is the current boundary. Research 119 remains the governing scope correction: ordinary ChatGPT should gain direct bounded access to authorized local-machine files through Codexless without requiring manual upload or a reasoning-model intermediary. Both first-class large-source channels are now end-to-end qualified on the same 78,874,939-byte read-only `51.Deep Learning2.annotated.pdf` source. Checkpoint 314 qualified direct page rendering into ChatGPT vision; Checkpoint 316 now qualifies direct embedded-text extraction through `codex.document_read`, returning all 17,999 page-16 characters with `pdfjs-dist@5.4.624`, no truncation, and the same source SHA-256. No Plugin refresh, Browser, OCR, source-workspace write, manual upload, or reasoning-model intermediary was used.
...
The private local-runtime repository is synchronized exactly to `origin/main` at `db046a49c57d2dc82c021e9f387f390a2b057331` with `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS` and `postflightOk=true`. It now preserves the completed Checkpoint 316 large-PDF text-read qualification in addition to the Checkpoint 315 candidate/publication evidence, the completed Checkpoint 314 page-render qualification, and earlier host-capability, Astra and Browser history.

The deterministic multi-native-PDF `document_file_link` qualification remains a real fallback, and both first-class large-source fallback channels are now fully qualified. On the same 78,874,939-byte Machine Learning source, `codex.document_render` delivers faithful page images directly to ChatGPT vision and `codex.document_read` delivers embedded text directly to ChatGPT text context. Page 16 returned 17,999 embedded-text characters with no truncation and the same source SHA-256 used by the visual qualification. The next direct-access work is automatic hybrid dispatch and policy generalization rather than another basic large-PDF transport proof.
...
The stable architecture now supports explicit register/update/remove of ordinary filesystem/project roots without another MCP schema publication. Per-workspace capability checks remain server-owned; semantic Git selects only `workspaceId`, derives branch/upstream dynamically, preserves the registered remote and integrity policy, and exposes no caller-selected cwd/URL/refspec/credentials/config/profile/sandbox/force inputs. Authenticated private Git is proven through the bounded host-network substrate. The latest preserved local-runtime boundary is `db046a49c57d2dc82c021e9f387f390a2b057331`. It preserves the completed large-PDF text-read qualification, including the 78,874,939-byte real-source PASS and direct 17,999-character page-16 extraction, while retaining the candidate/publication evidence, completed page-render qualification and earlier host-capability, Astra Phase 2 and GPT-5.6 Sol Browser evidence in private history. The latest private push passed `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS`, exact local/remote equality, clean tracked postflight, and `postflightOk=true`.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `large-pdf-text-read-qualified` boundary unless the repository has advanced further. The large-source text and visual routes are both now proven end to end on the same 78,874,939-byte Machine Learning source. Do not repeat either qualification. The next bounded research step is to generalize the automatic hybrid direct-source policy that selects among native whole-file handoff, native PDF splitting, embedded-text extraction and rendered-page vision based on the request and each route's independent envelope. For operational continuation steps, the collaborator must also resolve and read the governing procedure referenced by the active boundary before giving exact execution instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
