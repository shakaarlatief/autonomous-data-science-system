...
Checkpoint 324 is the current boundary. Research 119 remains the governing scope correction, and Research 120 governs the automatic hybrid PDF direct-source architecture. The private renderer-serialization candidate remains qualified at `ad61a5619165ec5675e75daecdb4fdb29ea6f19a` with 51/51 hybrid-PDF tests, `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS`, and `postflightOk=true`. The guarded live-publication package is now separately preflight-qualified: the first staged run exposed one stale installed `document-render-regression.mjs` expectation that still required one `[2,1]` child call; the matching regression expectation was updated to require two ordered one-page calls `[[2],[1]]` while preserving every read-only/limit/child-contract assertion. The final staging run passed all 51 private tests and all seven public regression scripts at the 60-tool preview.16 surface. Exact publication hashes are bound for both the live renderer (`f7f057...` -> `42199f...`) and render regression (`245813...` -> `3e60f...`), with atomic replacement, backups, post-write verification and rollback. No live Codexless file or process has yet been changed. The next legitimate step is to run the exact qualified helper from ordinary host PowerShell with `-Publish`, independently verify the two installed hashes, then follow the controlled restart in `docs/local_execution/OPERATIONS.md` before a fresh disposable intent-matrix retest and the >192 MiB facade-isolation proof.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `hybrid-pdf-render-serialization-publication-preflight-qualified` boundary unless the repository has advanced further. Research 120 is the governing automatic PDF architecture. Do not repeat the native split/native hybrid qualifications, the lower-level 192 MiB primitives, the failed five-call intent matrix, the serialization implementation, or its staged publication preflight as substitutes for advancing the boundary. The exact next Research 120 work is to run the already-qualified exact guarded helper from ordinary host PowerShell with `-Publish`, independently verify both installed hashes, preserve the source-published/restart-pending state, then follow the full controlled restart sequence in `docs/local_execution/OPERATIONS.md`, local/tunnel verification, and a fresh disposable `codex.pdf_access` intent-matrix retest. Only after the rendering-dependent branches pass should the public >192 MiB page/range-isolation qualification proceed through the same high-level facade. AB-008 remains relevant to fresh-chat schema acquisition. AB-020 remains relevant because the private integrity gate reached its 32 KiB tracked-path enumeration edge during Checkpoint 323 preservation. For operational continuation steps, the collaborator must resolve and read the governing procedure referenced by the active boundary before giving exact restart/publication instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
