...
Checkpoint 326 is the current boundary. Research 119 remains the governing scope correction, and Research 120 governs the automatic hybrid PDF direct-source architecture. The controlled restart has completed successfully: the restarted Codexless service reports `0.1.1-preview.16-hybrid-pdf-access`, 60 tools and `codexless-public-preview-v2`, while the restarted tunnel reports `/healthz` HTTP 200 and `/readyz` HTTP 200. The exact low-level request that failed before the fix, `codex.document_render` on `51.Deep Learning2.annotated.pdf` pages `[16,49]`, now succeeds live and returns both PNGs in one public request. Page 16 remains `583,130` bytes with SHA-256 `aca9cfadbfcb99382e22a2472495f26d1ab097922ce9406d66a8121810a56023`; page 49 remains `535,152` bytes with SHA-256 `2eaaa5f2ffaae9b3d1e171d200d412dc1c545425d5f539e48513ac25c4e5e927`. Both images were directly inspectable in ChatGPT and preserve the previously qualified content/fidelity. Repeated persistent-session renders returned the same hashes. This closes the low-level live activation defect. The next legitimate step is the fresh disposable five-call `codex.pdf_access` intent-matrix retest; only after that high-level matrix passes should the >192 MiB facade-isolation proof proceed.
