...
Checkpoint 330 is the current boundary. Research 120 remains qualified and closed for the accepted PDF direct-source routing scope, while Research 121 governs the non-PDF Office capability matrix. Before adding new tracked Office candidate files, the previously reproduced AB-020 private-integrity scaling defect has now been implementation-qualified. The private `runtime-private-bootstrap` gate no longer needs to return the complete tracked-file list through the generic 32 KiB `command/exec` capture envelope: the candidate runs one fixed read-only Node scanner inside the already-authorized sandbox, permits a separately bounded 4 MiB / 20,000-file internal enumeration, preserves the existing path/containment/regular-file/size/binary/secret-content checks, and emits only compact JSON to the parent policy. A regression deliberately crosses the old 32 KiB tracked-path boundary and passes. The candidate is privately preserved at `7e70bd05e4da76ff1ad260b2b1cbdc5c1d65a3fb`; its guarded publication preflight passes all eight focused tests and three staged 60-tool public regressions with no live file modified. The exact next step is ordinary-host publication of that qualified `semantic-git.mjs`, then the repository-governed controlled restart and one real above-32-KiB private semantic-push proof. Only after that live proof should new tracked `codex.file_link` Office implementation files be added.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `private-integrity-preflight-qualified-publication-next` boundary unless the repository has advanced further. Research 120 is complete for its accepted automatic PDF architecture scope; Research 121 governs the current Office whole-file handoff work. The exact next action is not yet adding Office files: first publish the already-qualified AB-020 `semantic-git.mjs` through its guarded ordinary-host helper, follow the full controlled restart sequence in `docs/local_execution/OPERATIONS.md`, then prove one real private semantic push while the tracked-path enumeration is above the old 32 KiB ceiling. Only after that live proof should new tracked `codex.file_link` source/tests be added for DOCX/PPTX/XLSX. Do not widen the generic `command/exec` output cap or weaken private secret/binary/content gates. Preserve the `--allow-addons` warning observed during cached PDF rendering as a hardening caveat. AB-008 remains relevant to fresh-chat schema acquisition. For operational continuation steps, the collaborator must resolve and read the governing procedure referenced by the active boundary before giving exact restart/publication instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
