...
Checkpoint 315 is the current boundary. Research 119 remains the governing scope correction: ordinary ChatGPT should gain direct bounded access to authorized local-machine files through Codexless without requiring manual upload or a reasoning-model intermediary. The large-source visual route remains end-to-end qualified from Checkpoint 314. The remaining first-class text gap is now addressed in source: `codex.document_read` retains its existing public schema but its bounded source ceiling has been raised from 32 MiB to 96 MiB, with tighter child-side large-input buffering. The published source passed syntax checks, focused 3/3 large-source tests and an installed valid 36,700,855-byte PDF.js 5.4.624 smoke test. The controlled runtime restart and real 78,874,939-byte Machine Learning page-16 text qualification are the only remaining steps.
...
The temporary exact-root `codexless-live` ordinary workspace admission used most recently for Checkpoint 315 publication was removed after verification. The durable registry is now revision 12 / content hash `e648cfb1aaa295f44ba5327181738a7f485ed9ebb5c5ef6deee193bd3eb07322` with only `ads-public`, `ads-local-runtime`, `big-data-statistics`, and `machine-learning`.

The private local-runtime repository is synchronized exactly to `origin/main` at `96f58fea13377943a0fbec2889384329bce49cb5` with `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS` and `postflightOk=true`. It now preserves the Checkpoint 315 large-PDF text-read candidate, guarded installed-source publication and installed dependency smoke evidence in addition to the completed Checkpoint 314 page-render qualification and earlier host-capability, Astra and Browser history.

The deterministic multi-native-PDF `document_file_link` qualification remains a real fallback, and the oversized-single-page visual route remains fully qualified. The corresponding first-class large-source text route is now source-published but not yet end-to-end live-qualified: the new `codex.document_read` implementation accepts bounded sources up to 96 MiB, preserves the existing direct embedded-text contract, and has already passed a valid 36.7 MB installed PDF.js smoke test. The currently running process still has the old module loaded. After the controlled restart, the exact discriminator is whether page 16 of the real 78,874,939-byte Machine Learning PDF returns embedded text directly through the existing tool. Automatic hybrid dispatch remains a later integration step.
...
The workspace registry is now revision `12`, content hash `e648cfb1aaa295f44ba5327181738a7f485ed9ebb5c5ef6deee193bd3eb07322`, with `ads-public`, `ads-local-runtime`, `big-data-statistics`, and `machine-learning`. Temporary exact-root `codexless-live` read/write admissions have been used only for bounded one-time live publication work, most recently Checkpoint 315, and removed immediately after publication/test verification. `big-data-statistics` has only `read` and `agent`; `machine-learning` remains strictly `read` only. Neither personal workspace has write, browser, or Git capability. Exact personal roots remain machine-local rather than public project authority.
...
The stable architecture now supports explicit register/update/remove of ordinary filesystem/project roots without another MCP schema publication. Per-workspace capability checks remain server-owned; semantic Git selects only `workspaceId`, derives branch/upstream dynamically, preserves the registered remote and integrity policy, and exposes no caller-selected cwd/URL/refspec/credentials/config/profile/sandbox/force inputs. Authenticated private Git is proven through the bounded host-network substrate. The latest preserved local-runtime boundary is `96f58fea13377943a0fbec2889384329bce49cb5`. It preserves the large-PDF text-read candidate, guarded installed-source publication, rollback identities and installed >32 MiB PDF.js smoke PASS while retaining the completed page-render qualification and earlier host-capability, Astra Phase 2 and GPT-5.6 Sol Browser evidence in private history. The latest private push passed `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS`, exact local/remote equality, clean tracked postflight, and `postflightOk=true`.
...
The live Codexless server is now `0.1.1-preview.15-host-capability-probe` with 59 source tools. The only new live capability relative to preview.14 is the temporary read-only MCP Apps host-capability diagnostic needed by Checkpoint 309; the GPT-5.6 Sol Browser candidate remains unpublished. The existing document paths, 96 MiB resource-link source ceiling, near-threshold host materialization evidence, native `auto_review`, and direct-Codex guard remain otherwise unchanged.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `large-pdf-text-read-source-published-restart-pending` boundary unless the repository has advanced further. The large-source text candidate is already source-published and has passed installed >32 MiB PDF.js smoke testing. Do not repeat that preparation. The exact next step is the controlled Codexless/tunnel restart from the governing operations runbook followed by same-chat `codex.document_read` qualification of `51.Deep Learning2.annotated.pdf` page 16 with `maxCharacters=25000`. No Plugin refresh is required because the public tool schema and count did not change. For operational continuation steps, the collaborator must also resolve and read the governing procedure referenced by the active boundary before giving exact execution instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
