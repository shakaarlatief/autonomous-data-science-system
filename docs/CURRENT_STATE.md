...
## Current active stage: controlled second large-PDF semantic worker after environment reconciliation

Checkpoint 307 is the current boundary. The first formal GPT-6 Astra worker on the held-out eight-page, 11,825,407-byte PDF completed terminally and fail-closed but produced no semantic answers because it could not establish the maintained PDF execution runtime with sufficient certainty. Under the preregistered Research 118 rules, Attempt 01 is `AMBIGUOUS`, not a semantic FAIL. Validation 066 preserves the exact task evidence and Checkpoint 307 preserves the reconciled continuation boundary. The source SHA-256 remained exact and no Browser, upload, external storage, dependency installation, or evaluator-key disclosure occurred.

Model-free follow-up localized the ambiguity rather than replaying the task. The maintained `pdf:pdf` Skill and primary-runtime bundle `26.904.11930` are present; primary-runtime Python 3.12.14 with `pdfplumber 0.11.9` / `pypdf 6.10.0` and Poppler 26.07.0 parse the exact source successfully, and `codex.document_read` independently returns all eight pages without truncation. A write-capable command from the nested scratch cwd reproduces `helper_unknown_error: setup refresh had errors`, while the same maintained `pdftoppm` executable succeeds from the registered ADS repository root and renders all eight pages to local scratch PNGs. A local scratch PNG also passed through `codex.image_read` into native ChatGPT vision.

The preferred document direction remains Browser-free. `codex.document_file_link` stays preferred within the clean-qualified host-materialization range, whose historical interval remains 7,417,428 bytes PASS to 7,993,210 bytes FAIL without claiming a universal threshold. Above that range, the semantic-worker direction remains viable because the first ambiguity is now localized to execution context rather than source corruption or missing maintained capabilities. The existing `codex.document_render` route remains qualified for its tested smaller-page envelope; this image-heavy fixture additionally reinforces keeping large rendered page bytes local rather than moving them through a serialized stdout/base64 command envelope.

The exact next Research 117 action is a controlled second formal Astra worker from the registered ADS repository-root cwd. The worker remains instruction-confined to `.tmp/astra-phase2-pdf-worker-01/source.pdf` and worker-created outputs in that scratch subtree, receives the exact maintained primary-runtime paths but not the evaluator key, and must independently parse, extract, render, visually inspect all eight pages, answer the held-out inventory/chart/scan questions, rehash the source, and return inspectable source-bound evidence. Only that changed-condition run may be classified `PASS / FAIL / AMBIGUOUS`; no blind retry has occurred.
...
The workspace registry is now revision `5`, content hash `0016f8ccaf84a7f9ff51884db4081856ed901d84891540e2f066b998e2628d01`, with `ads-public`, `ads-local-runtime`, `big-data-statistics`, and `machine-learning`. `big-data-statistics` has only `read` and `agent`; `machine-learning` remains strictly `read` only. Neither personal workspace has write, browser, or Git capability. Exact personal roots remain machine-local rather than public project authority.
