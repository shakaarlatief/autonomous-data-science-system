...
Checkpoint 317 is the current boundary. Research 119 remains the governing scope correction, and Research 120 now governs the automatic hybrid PDF direct-source architecture. The proven primitives remain unchanged, but their orchestration is now explicit: native whole-file handoff uses a conservative 7,000,000-byte host target; larger native-container requests use deterministic native parts; text and visual requests use the first-class direct routes within a 192 MiB processing envelope; and sources above that envelope are to be page/range-isolated before text extraction or rendering. Generated split PDFs are transport artifacts managed in a Codexless-owned content-addressed cache, not source-adjacent permanent files and not unconditional per-request regeneration.
...
The private local-runtime repository is synchronized exactly to `origin/main` at `fde55f1db3191086431f5a7d56fc416c1beebfa1` with `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS` and `postflightOk=true`. It now preserves the Research 120 hybrid-routing policy candidate and 192 MiB source-processing candidate in addition to the completed large-PDF text/render qualifications and earlier host-capability, Astra and Browser history.

The deterministic multi-native-PDF `document_file_link` qualification remains a real fallback, and both first-class large-source fallback channels remain fully qualified. Research 120 now defines their automatic route selection and derived-artifact lifecycle. A private policy-only candidate passes 8/8 route tests. Installed `document_read` and `document_render` source ceilings have also been raised from 96 MiB to 192 MiB; a valid 134,218,421-byte installed `pdfjs-dist@5.4.624` text smoke passes under a 384 MiB parser-child heap. Those source changes are published on disk but the running Codexless process still needs one controlled restart before the public tools expose the 192 MiB envelope. After restart, the next implementation seam is the managed artifact manager plus deterministic native splitter/page-range isolator.
...
The PDF parser is hardened out of the main Codexless event loop into a dedicated bounded Node child process with a 384 MiB V8 old-space ceiling, 30-second default hard deadline, bounded protocol output, Node permission restrictions, disabled PDF JavaScript evaluation/system-font lookup/worker fetch, and deterministic termination/recovery on timeout. The installed source now admits a bounded 192 MiB PDF snapshot and revalidates identity/size after reading; the parser child uses an exact declared-size input buffer and zero-copy `Uint8Array` view to avoid unnecessary whole-source duplication. The 192 MiB source change is published and statically/smoke-qualified but still requires the current controlled restart before public-tool live qualification.
...
The stable architecture now supports explicit register/update/remove of ordinary filesystem/project roots without another MCP schema publication. Per-workspace capability checks remain server-owned; semantic Git selects only `workspaceId`, derives branch/upstream dynamically, preserves the registered remote and integrity policy, and exposes no caller-selected cwd/URL/refspec/credentials/config/profile/sandbox/force inputs. Authenticated private Git is proven through the bounded host-network substrate. The latest preserved local-runtime boundary is `fde55f1db3191086431f5a7d56fc416c1beebfa1`. It preserves the Research 120 hybrid-routing policy candidate, the 192 MiB direct-processing source candidate, their tests and publication evidence, while retaining the completed large-PDF text/render qualifications and earlier host-capability, Astra Phase 2 and GPT-5.6 Sol Browser evidence in private history. The latest private push passed `RUNTIME_PRIVATE_BOOTSTRAP_SAFETY=PASS`, exact local/remote equality, clean tracked postflight, and `postflightOk=true`.
...
A new persistent conversation must allocate a fresh provider-local session/title, reconstruct public authority first, recover the relevant private local-runtime complement when accessible, and continue from the `hybrid-pdf-routing-architecture-accepted` boundary unless the repository has advanced further. Research 120 is the governing automatic PDF architecture. The 192 MiB text/render source changes are already published on disk and the policy-only router passes 8/8; do not redo that preparation. The exact operational next step is one controlled Codexless/tunnel restart so the public tools load the 192 MiB envelope. After that restart, implement the managed artifact manager plus deterministic native splitter/page-range isolator, with source-authority revalidation and cache-backed reuse/regeneration. For operational continuation steps, the collaborator must also resolve and read the governing procedure referenced by the active boundary before giving exact execution instructions; AB-022 preserves the currently observed gap while a stronger mechanism remains unaccepted. The broader `codexless-upstream-ecosystem-research` program remains active above that sub-boundary, and the preserved Source Vault ingestion route remains paused beneath the Level-2 research phase.
