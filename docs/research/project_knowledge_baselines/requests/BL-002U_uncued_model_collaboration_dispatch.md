...
Use only the exact frozen Git commit named above as project evidence for the task.

Do not use descendant/current repository content as evidence. In particular, do not read current Research 124, the failure corpus, baseline protocol, baseline result/evaluation files or later diagnosis records. Do not use web/external sources or prior-chat/project memory as evidence.
...
Do not perform any collaboration mutation yet. I want the intended immediate process first.

## Result receipt

After answering, preserve exactly the following in the user-supplied `RESULT_PATH`:
