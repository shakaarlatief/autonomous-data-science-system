...
Validation 150-152 / Checkpoints 393-395 deliberately supersede the parity-only creation target after the owner clarifies that Codexless should exceed the native connector where useful. Complete live UI capture yields 118 permission rows; the final initial profile selects 74 with Repository Administration(write), broad developer/security/organization capability, no enterprise authority, and explicit exclusion of secret-value and webhook families. The registration design gap is therefore closed again under the extended profile; owner App creation/install is now the only remaining pre-client-ID boundary.
...
The next substantive work is the owner-performed GitHub App creation/install using the frozen 74-row extended profile, followed by non-secret Client ID configuration, device-flow qualification, GraphQL sufficiency probes, and then the first read-only parity bundle plus separately gated extension actions. Authoritative GitHub API mapping continues per action bundle, while targeted native-wrapper qualification remains reserved for concrete material gaps that survive platform mapping.
