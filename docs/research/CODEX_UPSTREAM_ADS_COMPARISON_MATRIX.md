...
| MCP Apps `updateModelContext.resourceLink` | Current MCP Apps protocol allows a host to advertise `updateModelContext` with content modalities including `resourceLink`. Current OpenAI Plugin docs separately acknowledge files returned by tool file references, but do not establish a generic custom-MCP file-ID minting route. `A/H` | Checkpoint 309 live-published a read-only 59-tool host-capability probe that observes the actual ChatGPT host advertisement without moving local file bytes. The originating chat retained a stale callable projection after Plugin refresh. | This could be a cleaner direct-source path than PDF splitting if the fresh ChatGPT host actually advertises `resourceLink`, but protocol existence is not host availability and advertisement would not itself prove large-file success. | `MORE_EVIDENCE_REQUIRED` | Fresh disposable ChatGPT conversation: run the probe, classify `ADVERTISED` / `NOT_ADVERTISED`; if positive, qualify one tiny PDF before any large-file claim. |
