...
| MCP Apps `updateModelContext.resourceLink` | MCP Apps defines `updateModelContext` as host-advertised content modalities and `resourceLink?: {}` as the explicit resource-link indicator. `A` | Validation 068 fresh-host evidence shows `updateModelContext: {}` with no `resourceLink` property. Fresh tool projection and MCP App mount passed. The later `not_recorded` result is separately localized to request-local probe-store lifetime. `G` | The current ChatGPT host advertises model-context updates but not the `resourceLink` modality, so this direct-source candidate is unavailable on the current host advertisement. This does not establish a universal product limitation or say anything new about large-file materialization. | `KNOWN_HOST_LIMITATION` / `MONITOR` | Skip the tiny resourceLink model-context test. Re-probe only after relevant ChatGPT/MCP Apps host changes. |
| Deterministic multi-native-PDF handoff | Core MCP `resource_link` remains the same primitive for each part; no special upstream multipart reassembly primitive is required. `A/G` | Validation 069 qualifies the 11,825,407-byte eight-page source as three deterministic native PDFs below the clean host PASS envelope. Render fidelity is 8/8 exact, `document_file_link` succeeds 3/3, the host materializes all three files, and ChatGPT inspects/reasons across the complete source. | This is now a proven direct-source fallback for oversized PDFs, but it is weaker than unchanged whole-file handoff for document-wide semantics and single-file UX. | `KEEP_AS_FALLBACK_CANDIDATE` | Decide productization and document-container safeguards; retain Browser upload only as a heavier fallback/comparison path. |
...
| Direct Browser lifecycle outside a genuine turn | Direct `mcpServer/tool/call` dispatches out of band through `CodexThread::call_mcp_tool(...)`; current Browser cleanup tracks `{session_id, turn_id}` through Node REPL turn-ended hooks, while real `turn/start` owns the normal terminal turn lifecycle. `A/B/G` | Live preview.47 restores current Browser discovery but blocks every existing-tab claim before dispatch because the direct path has no supported release/unclaim primitive and its synthetic turn metadata does not create a genuine turn. | Do not fabricate `turn_ended`, infer release from synthetic metadata, treat `markDeliverable`/`markHandoff` as release, or rely on process teardown. Keep direct claim actions fail-closed; qualify a separate genuine-turn Browser path only if the capability warrants a metered model turn. | `KEEP_ADS` / `MORE_EVIDENCE_REQUIRED` | Validation 208 / Checkpoint 532 localize the exact boundary. Next proof, if prioritized: one bounded genuine-turn Browser claim/release/interruption experiment. |
| Browser plugin discovery provenance | Current installed-plugin discovery exposes the maintained bundled Chrome/Browser pair; current local build is `26.908.40834`. `B/G` | Preview.47 now live-binds exact same-build manifests, trusted-cache realpath containment, Browser client SHA-256 and Browser service SHA-256, retains the legacy Skill only as fallback, and restarts on same-path byte drift. | The two Astra Phase 2 discovery weaknesses are repaired in the live compatibility layer without patching upstream Browser/Chrome bundles or widening Browser authority. | `ADS_ALREADY_SOLVES` / `KEEP_ADS` | Validation 207 / Checkpoint 531 live-qualified status and tab inventory; Validation 208 preserves the separate lifecycle limitation. |
