...
**Status:** RESEARCHING / NATIVE SPLITTER QUALIFIED BUT NOT UNIVERSAL / HYBRID OVERSIZED-PAGE ROUTE NEXT
**Priority:** P1
...
Checkpoint 309 / Validation 067 introduced the narrow direct-host discriminator before PDF splitting, and Checkpoint 310 / Validation 068 resolved it: the current ChatGPT host advertises `updateModelContext` but not its `resourceLink` modality. Checkpoint 311 / Validation 069 then qualified the deterministic multi-native-PDF fallback end to end on the 11,825,407-byte eight-page source. Checkpoint 312 / Validation 070 now narrows that result against the larger read-only Machine Learning PDFs: several files contain individual pages whose native one-page PDFs exceed the host envelope, including pages around 14-17.6 MB. A page-boundary splitter therefore cannot be universal. The next direct-access work is a hybrid model-free oversized-single-page route that can isolate a page internally and expose faithful page text plus visual/source representation without writing back to the source workspace or invoking a semantic model.
