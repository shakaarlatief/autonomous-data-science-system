...
**Status:** RESEARCHING / LARGE-PDF PAGE-RENDER QUALIFIED / HYBRID POLICY + LARGE-TEXT PATH NEXT
**Priority:** P1
...
Checkpoint 309 / Validation 067 introduced the narrow direct-host discriminator before PDF splitting, and Checkpoint 310 / Validation 068 resolved it: the current ChatGPT host advertises `updateModelContext` but not its `resourceLink` modality. Checkpoint 311 / Validation 069 then qualified the deterministic multi-native-PDF fallback end to end on the 11,825,407-byte eight-page source. Checkpoint 312 / Validation 070 narrowed that result against the larger read-only Machine Learning PDFs: several files contain individual pages whose native one-page PDFs exceed the host envelope. Checkpoint 313 / Validation 071 preserved the first bounded oversized-page response and renderer-only 96 MiB source-admission change. Checkpoint 314 / Validation 072 now closes that route end to end: after controlled restart, page 16 from the 78,874,939-byte Deep Learning 2 source reached ChatGPT directly as a 583,130-byte standard MCP PNG under read-only authority, with concrete StyleGAN visual inspection and no Browser, OCR, manual upload, source-workspace write or semantic model. The next direct-access work is hybrid-policy generalization, especially a first-class large-source text path and automatic fallback selection.
