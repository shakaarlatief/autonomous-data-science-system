...
const rowsHost = document.querySelector('#tag-motion-rows')
const practicalHost = document.querySelector('#tag-motion-practical-nodes')
const summary = document.querySelector('#tag-motion-summary')
...
const motionVariants = [
  {
    ...
    description: 'No semantic motion. Establishes whether animation improves the runtime tag at all.',
  },
  {
    id: 't1',
    code: 'T1',
    label: 'Short Tracer',
    description: 'The current refinement: one small luminous segment travels around the fixed perimeter.',
  },
  {
    id: 't2',
    code: 'T2',
    label: 'Comet Flow',
    description: 'A brighter head with a longer fading wake creates continuous flow without moving the box itself.',
  },
  {
    id: 't3',
    code: 'T3',
    label: 'Perimeter Current',
    description: 'A restrained dash pattern moves through the whole outline so the border itself reads as flowing.',
  },
  {
    id: 't4',
    code: 'T4',
    label: 'Twin Orbit',
    description: 'Two opposite highlights circulate together, creating balanced motion with less empty travel.',
  },
  {
    id: 't5',
    code: 'T5',
    label: 'Long Glide',
    description: 'One longer highlighted band glides slowly around the perimeter for a calmer premium effect.',
  },
  {
    id: 't6',
    code: 'T6',
...
const practicalFixture = [
  {
    key: 'q', category: 'question', disposition: 'current', runtime: 'human',
    title: 'Approve target definition', subtitle: 'Current work needs an explicit human decision',
  },
  {
    key: 'i', category: 'investigation', disposition: 'current', runtime: 'running',
    title: 'Production missingness', subtitle: 'Current investigation is executing now',
  },
  {
    key: 'v', category: 'validation', disposition: 'recommended', runtime: 'queued',
    title: 'Chronological validation', subtitle: 'Recommended next check has already been queued',
  },
  {
    key: 'm', category: 'model', disposition: 'current', runtime: 'failed',
    title: 'Boosted candidate', subtitle: 'Current training attempt failed',
  },
  {
    key: 'e', category: 'evaluation', disposition: 'deferred', runtime: 'none',
    title: 'Calibration review', subtitle: 'Deferred work has no current runtime episode',
  },
  {
    key: 'f', category: 'investigation', disposition: 'future', runtime: 'none',
    title: 'Drift investigation', subtitle: 'Future work has no current runtime episode',
  },
]
...
function renderRows() {
  ...
  rowsHost.innerHTML = motionVariants.map((variant) => `
    <article class="tag-motion-row" data-runtime="running" style="--runtime-rgb:${runtimeStates.running.rgb};">
      <div class="tag-motion-label">
      ...
      ${nodeMarkup({
        ...
        subtitle: 'Current project work with a live execution episode',
        extraClass: 'tag-motion-node',
      })}
    </article>
  `).join('')
}
...
function renderPracticalScene() {
  ...
  practicalHost.innerHTML = practicalFixture.map((item) => nodeMarkup({
    ...
    extraClass: `tag-motion-practical-node scene-${item.key}`,
    key: item.key,
  })).join('')
}

function nodeMarkup({ category, disposition, runtime, motion, title, subtitle, extraClass, key = '' }) {
  const meta = categoryMeta[category]
  const projectState = dispositions[disposition]
  const runtimeState = runtimeStates[runtime] || runtimeStates.none
  const runtimeTag = runtime === 'none' ? '' : `
    <span class="motion-runtime-tag" aria-hidden="true">
      <svg viewBox="0 0 100 34" preserveAspectRatio="none">
        <rect class="tag-base" x="1.4" y="1.4" width="97.2" height="31.2" rx="10.5" pathLength="100"></rect>
        <rect class="tag-flow tag-flow-a" x="1.4" y="1.4" width="97.2" height="31.2" rx="10.5" pathLength="100"></rect>
        <rect class="tag-flow tag-flow-b" x="1.4" y="1.4" width="97.2" height="31.2" rx="10.5" pathLength="100"></rect>
        <rect class="tag-flow tag-flow-c" x="1.4" y="1.4" width="97.2" height="31.2" rx="10.5" pathLength="100"></rect>
      </svg>
      <span>${runtimeState.code}</span>
    </span>
  `
...
function setupControls() {
  for (const button of document.querySelectorAll('button[data-tag-motion]')) {
    button.addEventListener('click', () => {
      html.dataset.practicalTagMotion = button.dataset.tagMotion
      for (const node of document.querySelectorAll('.tag-motion-practical-node')) {
        node.dataset.tagMotion = button.dataset.tagMotion
      }
      updateControls()
    })
  }
...
function setupInteractions() {
  for (const node of document.querySelectorAll('.tag-motion-node, .tag-motion-practical-node')) {
    const practical = node.classList.contains('tag-motion-practical-node')
...
function setupGeometry() {
  ...
  if ('ResizeObserver' in window) {
    ...
    const world = document.querySelector('#tag-motion-practical-world')
...
function updatePracticalGeometry() {
  const world = document.querySelector('#tag-motion-practical-world')
  const svg = document.querySelector('.tag-motion-relations')
  if (!world || !svg) return
